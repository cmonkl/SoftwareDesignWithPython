...
def plot_rank_exec():
    ...
    for i, (key, value) in enumerate(ranked_dict.items()):
        print(str(key).ljust(pad+2), str(i + 1).ljust(pad+2), str(value).ljust(pad+2))
...
class Decorator_2(Decorator_1):
    ...
    def __call__(self, *args, **kwargs):
        ...
        with open("exec_info.txt", 'a+') as f:
            ...
            f.write('Doc: '.ljust(pad - 4) + add_spaces(self.func.__doc__[1:] if self.func.__doc__ is not None else "\n", 11))
            f.write('\nSource: '.ljust(pad) + add_spaces(inspect.getsource(self.func)) + '\n')
